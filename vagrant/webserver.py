...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path.endswith("/delete"):
                #this is all extracting the info from the POST request
                ...
                if myRestaurantQuery != [] :
                    #this is updating the database accordingly
                    print "inside the if statement"
                    session.delete(myRestaurantQuery)
                    print "deleting %s" % myRestaurantQuery.name
                    session.commit()
                    #this next part is sending a new request to ourselves so the restaurants page will be displayed
                    self.send_response(301)
                    self.send_header('Content-type',    'text/html')
                    self.send_header('Location', '/restaurants')
                    self.end_headers()
